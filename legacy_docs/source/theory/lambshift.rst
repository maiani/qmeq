...
where :math:`\mathcal{D}` is the dissipator built from the jump operators
:math:`T^{l}_{ba}` of Appendix F of the
`QmeQ paper <https://doi.org/10.1088/1361-648X/aa9c15>`_, and :math:`H_{LS}` is the
...
states :math:`b`, :math:`b'`, :math:`b''` carry :math:`N` electrons, the states
:math:`a` carry :math:`N-1`, and the states :math:`c` carry :math:`N+1` electrons.
All expressions are written in the eigenbasis of :math:`H_{QD}`, in which the
tunneling amplitudes are the rotated matrix elements :math:`T^{l}_{ba}` (these
already contain the factor :math:`1/2\pi`, so that
:math:`\sum_{j}|t_{lj}|^{2}=\Gamma_{l}/2\pi`).

Lamb shift Hamiltonian
----------------------

Beyond the secular approximation, i.e. keeping all energy differences
:math:`\omega_{bb'}=E_b-E_{b'}` of the spectrum and not only those of neighbouring
levels, the Lamb shift of lead :math:`l` is
...
with :math:`H_{LS}=\sum_{l}H_{LS}^{l}`.  The two sums are the particle and the hole
...
and :math:`\tilde{\Lambda}_{l}(E)=\Lambda_{l}(E)|_{\mu_l\to-\mu_l}`, where
:math:`\psi` is the digamma function.  The right hand side is the standard wide-band
expansion, the same approximation that `itype=1` uses for the principal parts of the
1vN, Redfield and RTD kernels.

:math:`H_{LS}` is Hermitian and block diagonal in the charge, as it must be because
...
:func:`~qmeq.specfunc.specfunc.func_lambshift` returns only the digamma term and
drops the bandwidth constant :math:`-\ln(D/2\pi T_{l})`.  The constant is the same for
every pair of states, so its contribution to :math:`H_{LS}` is proportional to
...
where :math:`X_{l}=\sum_{j}t_{lj}d_{j}` is the tunneling operator of lead :math:`l`.
...
:math:`[H_{LS},\rho]`.  This holds for a symmetric wide band; the residual asymmetry
:math:`\ln|D_-/D_+|` between the particle and hole factors is beyond the accuracy of
the digamma approximation itself.

Contribution to the kernel
--------------------------

QmeQ stores the kernel :math:`K` of the master equation such that the free evolution
of a coherence is :math:`K_{(bb'),(bb')}=E_b-E_{b'}` (see
:meth:`~qmeq.approach.kernel_handler.KernelHandler.set_energy`), i.e. :math:`K=iL`
for :math:`\dot{\rho}=L\rho`.  Writing out the commutator,
...
which is what :meth:`~qmeq.approach.base.lindblad.ApproachLindblad.generate_coupling_terms`
...
:math:`\varepsilon` coupled to one lead, the occupied state is shifted by
:math:`\delta E_1 = |t|^{2}\,\Lambda(\varepsilon)` and the empty state by
:math:`\delta E_0 = |t|^{2}\,\tilde{\Lambda}(-\varepsilon)`, which is the same
convention in which the real parts of the 1vN and Redfield principal parts
(:func:`~qmeq.specfunc.specfunc.func_1vN`) enter their kernels.  The test suite
verifies that the level renormalisation extracted from :math:`H_{LS}` agrees with the
1vN factors.
...
are dropped.  See :doc:`transport_options` for the complete cross-approach
compatibility table.
